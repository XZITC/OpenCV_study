...
def pipline(src, left_line, right_line, debug_flag):  # frame也代表了可以用图
    ...
    binary_final_roi, binary_final_extra_roi, gray_fianl_roi, deal_image_roi, Mvt = fc.selec_roi(point_x, point_y,
                                                                                                 edge_img,
                                                                                                  blur_resize_img, 0,
                                                                                                 1)
    # 这边做一个颜色定时的功能
    lane_color = fc.find_lane_key_color(gray_fianl_roi, deal_image_roi, binary_final_roi, debug_flag)
    lane_binary_roi_img = fc.extra_object_demo(deal_image_roi, lane_color, debug_flag)
